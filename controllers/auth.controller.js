...
const adminLogin = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      error.message || "Error While Login As Admin",
      500,
    );
  }
};
...
const technicianLogin = async (req, res) => {
  try {
    ...
    return successResponse(
      res,
      null,
      user.message || "OTP sent to your mobile",
      200,
    );
  } catch (error) {
    return errorResponse(
      res,
      error.message || "Error While Login As Technician",
      500,
    );
  }
};

const technicianLogin_new = async (req, res) => {
  try {
